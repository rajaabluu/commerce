...
func (s *UserService) GetAuthenticatedUser(ctx context.Context, ID uint) (*model.UserResponse, error) {
	tx := s.DB.WithContext(ctx)
...
func (s *UserService) UpdateProfile(ctx context.Context, req *model.UpdateUserProfileRequest) (*model.UserResponse, error) {
	tx := s.DB.WithContext(ctx).Begin()
	defer tx.Rollback()
